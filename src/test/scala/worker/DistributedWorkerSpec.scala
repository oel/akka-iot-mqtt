...
object DistributedWorkerSpec {
  ...
  class FlakyWorkProcessor extends Actor {
    var i = 0
...
class DistributedWorkerSpec(_system: ActorSystem)
  ...
  "Distributed workers" should "perform work and publish results" in {
    ...
    val clusterClient = workerSystem.actorOf(
      ...
      "clusterClient")
    for (n <- 1 to 3)
      workerSystem.actorOf(Worker.props(clusterClient, Props[WorkProcessor], 1.second), "worker-" + n)
    val flakyWorker = workerSystem.actorOf(Worker.props(clusterClient, Props[FlakyWorkProcessor], 1.second), "flaky-worker")

    Cluster(system).join(clusterAddress)
    clusterProbe.expectMsgType[MemberUp]
    val iotagent = system.actorOf(Props[IotAgent], "iotagent")
    ...
    for (n <- 2 to 100) {
      iotagent ! Work(n.toString, lamp, n)
      expectMsg(IotAgent.Ok)
    }
