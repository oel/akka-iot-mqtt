...
object Main {
  ...
  def startBackend(port: Int, role: String): Unit = {
    ...
    system.actorOf(
      ...
      "master")
    system.actorOf(Props[PostProcessor], "postprocessor")
  }

  def startIotAgent(port: Int): Unit = {
    val conf = ConfigFactory.parseString("akka.remote.netty.tcp.port=" + port).
      withFallback(ConfigFactory.load("iotagent"))
    ...
    val deviceRequest = system.actorOf(Props(classOf[DeviceRequest], mqttPubSub), "devicerequest")
    val iotAgent = system.actorOf(IotAgent.props(clusterClient, mqttPubSub, deviceRequest), "iotagent")
  }
